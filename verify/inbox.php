...
$sql = "SELECT *, DATE_FORMAT(date_sent, '%b %e, %Y %H:%i:%s') AS date_sent FROM user_inbox WHERE to_id='$to_id' ORDER BY id DESC LIMIT $offset, $rowsPerPage";
$result = mysqli_query($db, $sql);

// Get the total number of rows for pagination
$totalRows = mysqli_num_rows(mysqli_query($db, "SELECT * FROM user_inbox WHERE to_id='$to_id'"));
        ...
        <div class="info">
            <div class="container my-5" style="text-align: center;">
                ...
                <form action="deleteMessage" method="post">
                    ...
                    <h1 style="color: #ffffff;text-align: center;padding: 10px"> Inbox</h1>
                    <button class="btn btn-secondary" style="margin-top: 5px;margin-bottom: 5px;margin-left:5px;color: #ffffff;">
                            <a href="sent" class="link-light btn-no-underline">Sent</a>
                    </button>
                    <button class="btn btn-secondary" style="margin-top: 5px;margin-bottom: 5px;color: #ffffff;">
                            <a href="compose_message?id=<?php echo $_SESSION['current'];?>" class="link-light btn-no-underline">Message Admin</a>
                    </button>
                    <button type="submit" class="btn btn-danger" style="margin-top: 5px;margin-bottom: 5px; margin-left:1px"><i class="fas fa-trash-alt"></i></button>
                        ...
                        <table class="table table-hover shadow">
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                echo '<tr>';
                                echo '<td width="5%">' . ($row['read_receipt'] != 0 ? "<i class=\"fas fa-circle fa-sm\"></i>" : "")  . '</td>';
